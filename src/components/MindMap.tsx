...
function MindMapInner() {
  ...
  // Update node data when selection or loading changes
  useEffect(() => {
    setNodes(nds =>
      nds.map(node => ({
        ...node,
  ...
  const handleInputSubmit = useCallback(async (word: string) => {
    ...
    // Calculate position for new center node
    let centerX = 0;
    let centerY = 0;

    if (selectedNodesList.length > 0) {
      // If there are selected nodes, place near them
      ...
      // If there are existing nodes, place to the right
    ...
    try {
      const response = await fetch('/api/generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ word: `翻译成英文: ${word}` }),
      });

      // Simple translation - just update with the word itself if API fails
      setNodes(nds =>
        nds.map(n =>
          n.id === newNodeId
            ? { ...n, data: { ...n.data, english: word, isLoading: false } }
            : n
        )
      );
    } catch {
  ...
  return (
    <div className="w-screen h-screen bg-white relative">
      ...
      <motion.div
        ...
        animate={{ opacity: 1 }}
        transition={{ delay: 1 }}
        className="fixed bottom-24 left-1/2 -translate-x-1/2 text-center text-sm text-gray-400"
      >
